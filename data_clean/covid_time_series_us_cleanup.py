...
ts_confirmed_us = pd.read_csv("time_series_covid19_confirmed_US.csv")
ts_deaths_us = pd.read_csv("time_series_covid19_deaths_US.csv")

## Clean up unnessary columns

def data_sort(covid_df, state):
    covid_df = covid_df.drop(columns=["UID","iso2","iso3","code3","FIPS","Admin2","Country_Region","Lat","Long_","Combined_Key"])
    ...
    cols = ["Province_State"]
    covid_df = pd.melt(covid_df, id_vars=cols, value_vars=dates)
    covid_df.rename(columns={'variable': 'report_date', 'value': state}, inplace=True)
    covid_df1 = covid_df.groupby(["report_date","Province_State"])[state].sum().reset_index()
    covid_df1["report_date"]=covid_df1["report_date"].apply(lambda x: pd.to_datetime(x).strftime('%Y-%m-%d'))
    covid_df1 = covid_df1.sort_values(by=['report_date', 'Province_State'])

    return covid_df1

ts_confirmed_us1 = data_sort(ts_confirmed_us, "confirmed")
ts_deaths_us1 = data_sort(ts_deaths_us, "deaths")

ts_us_cleaned = pd.merge(ts_confirmed_us1, ts_deaths_us1,  how='inner', on=["Province_State","report_date"])
ts_us_cleaned.rename(columns={'Province_State':'state'}, inplace=True)
ts_us_cleaned.to_csv(r'us_data_cleaned.csv', index=False)

with open('us_covid_data_cleaned.sql', 'w') as f:
    for index, row in ts_us_cleaned.iterrows():
        f.write('insert into us_covid_cases (`state`,`report_date`,`confirmed`,`deaths`)'
                ' values ("{}","{}",{},{});\n'.format(row['state'],row['report_date'],row['confirmed'],row['deaths']))
f.close()
